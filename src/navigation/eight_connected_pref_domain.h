...
#ifndef EIGHT_CONNECTED_PREF_DOMAIN_H
#define EIGHT_CONNECTED_PREF_DOMAIN_H

namespace navigation {

struct EightConnectedPrefDomain {
  ...
  float EdgeCost(const State& s1, const State& s2) const {
    if (s1.x() == s2.x() || s1.y() == s2.y()) {
      return 0.5 * costmap.at<int>(s1.y(), s1.x()) + 0.5 * costmap.at<int>(s2.y(), s2.x());
    } else {
      return kDiagonalLength * (0.5 * costmap.at<int>(s1.y(), s1.x()) + 0.5 * costmap.at<int>(s2.y(), s2.x()));
    }
  }
  ...
  float EdgeCost(const uint64_t k_s1, const uint64_t k_s2) const {
    ...
    if (x1_int == x2_int || y1_int == y2_int) {
      ...
      return kDiagonalLength * (0.5 * costmap.at<int>(y1_int, x1_int) + 0.5 * costmap.at<int>(y2_int, x2_int));
    }
  }
...
#endif  // EIGHT_CONNECTED_PREF_DOMAIN_H
