...
#define V2COMP(v) v.x(), v.y()
    ...
    // TODO <Sadanand> Figure out solution to your issue on github: why following should ever be happening!
...
string Navigation::GetNavStatus() {
    switch (nav_state_) {
        ...
        case NavigationState::kContingency: {
            return "Contingency";
        } break;
...
static const int CONTINGENCY_HALT_TEMP = 500;
static int contingency_halt_counter = 0;
...
bool Navigation::Run(const double &time,
    ...
    if (!initialized_) {
        if (kDebug)
            printf("Parameters and maps not initialized\n");
        return false;
    }
    if (!odom_initialized_) {
        if (kDebug)
            printf("Odometry not initialized\n");
        return false;
    }
    ...
    if (contingency_enabled_) {
        ...
        if (contingency_halt_counter < CONTINGENCY_HALT_TEMP) {
            contingency_halt_counter++;
            Halt(cmd_vel, cmd_angle_vel);
            return true;
    ...
    // TODO <Sadanand>: simplify the logic to if-else statements for code readability
    ...
    if (nav_state_ == NavigationState::kPaused ||
        ...
        if (kDebug)
            printf("Reached Goal: TurnInPlace\n");
        TurnInPlace(cmd_vel, cmd_angle_vel);
    }
