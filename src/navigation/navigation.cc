...
void Navigation::Initialize(const NavigationParameters& params,
    ...
    costmap_ = nav2_costmap_2d::Costmap2D(local_costmap_size, local_costmap_size, params_.local_costmap_resolution, -params.local_costmap_size / 2, -params.local_costmap_size / 2);
    ...
    global_costmap_ = nav2_costmap_2d::Costmap2D(global_costmap_size_x, global_costmap_size_y, params_.global_costmap_resolution, params.global_costmap_origin_x, params.global_costmap_origin_y);
...
void Navigation::LoadVectorMap(const string& map_file) {  // Assume map is given as MAP.navigation.json
                    ...
                    for (int k = -cell_inflation_size; k <= cell_inflation_size; k++) {
                        ...
                        if ((cell_dist <= cell_inflation_size) && (mx + j >= 0) && (mx + j < x_max) && (my + k >= 0) && (my + k < y_max)) {
                            ...
                            if (j == 0 && k == 0) {
                                ...
                                cost = std::ceil(std::exp(-1 * params_.inflation_coeff * (dist - params_.min_inflation_radius)) * (nav2_costmap_2d::INSCRIBED_INFLATED_OBSTACLE - 1));
                            }
    ...
    for (const auto& pair : inflation_cells) {
        ...
        // global_costmap_.setCost(mx, my, nav2_costmap_2d::LETHAL_OBSTACLE);
    ...
    if (params_.do_intermed) {
        ...
        while (!intermediate_queue.empty()) {
            ...
            for (size_t i = 0; i < neighbors_x.size(); i++) {
                ...
                if (new_row >= 0 && new_row < static_cast<int>(costmap_.getSizeInCellsX()) && new_col >= 0 && new_col < static_cast<int>(costmap_.getSizeInCellsY())) {
                    double wx, wy;
...
bool Navigation::IntermediatePlanStillValid() {
    ...
    if ((nav_goal_loc_ - plan_path_[0].loc).norm() > sqrt(2 * params_.local_costmap_resolution) / 2 && (robot_loc_ - plan_path_[0].loc).norm() < 1) {
        return false;
    }
    ...
    for (size_t i = 0; i < plan_path_.size(); i++) {
        ...
        if (in_map && (costmap_.getCost(mx, my) == nav2_costmap_2d::LETHAL_OBSTACLE || costmap_.getCost(mx, my) == nav2_costmap_2d::INSCRIBED_INFLATED_OBSTACLE)) {
            return false;
        }
    }
    return true;
...
bool Navigation::Run(const double& time,
    ...
    if (params_.do_intermed) {
        ...
        struct PointComparison {
            ...
            PointComparison(const nav2_costmap_2d::Costmap2D map, const int robot_x, const int robot_y) : costmap(map), robot_mx(robot_x), robot_my(robot_y) {}
        ...
        while (iter != sorted_obstacle_cells.end() && iter != std::prev(sorted_obstacle_cells.end())) {
            nav2_costmap_2d::MapLocation point_a;
            costmap_.indexToCells(*iter, point_a.x, point_a.y);
            nav2_costmap_2d::MapLocation point_b;
            costmap_.indexToCells(*std::next(iter), point_b.x, point_b.y);
            nav2_costmap_2d::MapLocation robot_point;
            robot_point.x = robot_mx;
            robot_point.y = robot_my;
            robot_point = robot_point;
            vector<nav2_costmap_2d::MapLocation> polygon = {point_a, point_b, robot_point};
            vector<nav2_costmap_2d::MapLocation> fill_cells;
    ...
    if (nav_state_ == NavigationState::kGoto ||
        ...
        if ((!params_.do_intermed && !PlanStillValid()) || (params_.do_intermed && (!PlanStillValid() || !IntermediatePlanStillValid()))) {
