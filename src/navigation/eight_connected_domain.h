...
struct EightConnectedDomain {
    ...
    EightConnectedDomain(float size_x,
                         ...
                         float resolution,
                         const std::string& map_file) : kResolution(resolution),
    ...
    void CheckAndAdd(const State& s,
        ...
        if (dx < 0 && int64_t(x_int) < dx)
            return;
        else if (x_int + dx >= kMapWidthInt)
            return;
        if (dy < 0 && int64_t(y_int) < dy)
            return;
        else if (dy == 1 && y_int + dy >= kMapHeightInt)
            return;
