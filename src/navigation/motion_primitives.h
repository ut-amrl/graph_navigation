...
struct PathRolloutBase {
    ...
    virtual void GetControls(const navigation::MotionLimits& linear_limits,
                             ...
                             const Eigen::Vector2f& linear_vel, const float angular_vel, Eigen::Vector2f& vel_cmd,
...
struct PathRolloutSamplerBase {
    ...
    virtual void Update(const Eigen::Vector2f& new_vel, const float new_ang_vel,
                        const Eigen::Vector2f& new_local_target, const std::vector<Eigen::Vector2f>& new_point_cloud) {
        vel = new_vel;
        ang_vel = new_ang_vel;
...
struct PathEvaluatorBase {
    ...
    // navigation target, obstacle point cloud, and any other factors relevant for
    // local navigation planning.
    virtual void Update(const Eigen::Vector2f& new_loc, const float new_ang, const Eigen::Vector2f& new_vel,
                        const float new_ang_vel, const Eigen::Vector2f& new_local_target,
