...
public:
    ...
    : nh_(nh), params_(params)
  {
    ...
    //    (Must use a ros::Subscriber, not image_transport::Subscriber, for /compressed topics)
    ...
    ROS_INFO_STREAM("Warping images from [" << params_.image_topic << "] "
                    "to BEV [" << params_.bev_topic << "] "
                    "with output size " << params_.bev_image_width << "x" << params_.bev_image_height);
  }
...
private:
  void imageCallback(const sensor_msgs::CompressedImageConstPtr& msg)
  {
    ...
    try
    {
      // Construct a temporary cv::Mat header for the compressed bytes in msg->data
      ...
      bev_msg.header = msg->header; // Use same timestamp and frame_id if desired
...
void LoadConfig(ImageToBEVParameters& params)
{
  ...
  if (!fs.isOpened()) {
    ROS_ERROR_STREAM("Failed to open calibration file: " << params.calibration_file);
    ros::shutdown();
    return;
  }
  ...
  if (!node.empty()) {
    ...
    ROS_INFO("Loaded homography from file: %s", params.calibration_file.c_str());
  } else {
