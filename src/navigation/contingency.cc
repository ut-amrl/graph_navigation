...
Eigen::Vector2f Navigation::GetSafeLocalLocFromGround(const Eigen::Vector2f &gr) {
    return Rotation2Df(-robot_angle_) * (gr - robot_loc_);
}

void Navigation::GetSafeGroundLocFromLocal(Eigen::Vector2f &gr, const Eigen::Vector2f &loc) {
    gr = Rotation2Df(robot_angle_) * loc + robot_loc_;
}
...
bool Navigation::ContingencyPlanner(const Eigen::Vector2f &initial, Eigen::Vector2f &cmd_vel, float &angular_vel_cmd, const bool &kDebug) {
    ...
    if (GetSafeLocalLocFromGround(safe_anchor_ground_target_loc_).squaredNorm() < Sq(params_.target_dist_tolerance) && robot_vel_.squaredNorm() < Sq(params_.target_vel_tolerance) && AngleDist(robot_angle_, safe_anchor_ground_target_angle_) < params_.target_angle_tolerance) {
        ...
        if (!FLAGS_no_local) {
            if (fabs(theta) > params_.local_fov || (GetSafeLocalLocFromGround(safe_anchor_ground_target_loc_).squaredNorm() < Sq(params_.target_dist_tolerance) && robot_vel_.squaredNorm() < Sq(params_.target_vel_tolerance))) {
                if (kDebug)
                    printf("TurnInPlace\n");
