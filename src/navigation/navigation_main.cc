...
#define BOOL_PARAM(x) CONFIG_BOOL(x, "NavigationParameters." #x);
  ...
  params->recovery_type = CONFIG_recovery_type;
...
// ROS Specific global declarations
  ...
  std::string deprecated_path = GetDeprecatedMapPath(FLAGS_maps_dir, FLAGS_map);
  ...
  navigation->InitializeOSM(osm_planner_params);
